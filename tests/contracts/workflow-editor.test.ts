...
import {
  ...
  emptyWorkflow,
  needsWorkflowLayout,
} from "../../apps/console/src/features/workflows/model/workflow-model.ts";
...
test("line insertion preserves branch topology and leaves geometry to the editor without partial mutation", async () => {
  const { insertWorkflowNode } = await import(
    "../../apps/console/src/features/workflows/model/workflow-editing.ts"
  );
...
test("pasting selected nodes rewrites internal references while preserving literals and external references", async () => {
  const { pasteWorkflowNodes } = await import(
    "../../apps/console/src/features/workflows/model/workflow-editing.ts"
  );
...
test("canvas document conversion preserves metadata and geometry while rejecting mismatched nodes and ports", async () => {
  const { documentFor, captureDocument } = await import(
    "../../apps/console/src/features/workflows/canvas/document.ts"
  );
  const source = emptyWorkflow("文档往返");
  source.layout = { start: { x: 32, y: 64 }, end: { x: 480, y: 64 } };
  const doc = documentFor(source);
  assert.deepEqual(captureDocument(source, doc), source);
  doc.nodes[0].id = "wrong";
  assert.throws(() => captureDocument(source, doc), /标识不一致/);
  const invalid = documentFor(source);
  assert.ok(invalid.edges?.[0]);
  invalid.edges[0].sourcePortID = "invalid";
  assert.throws(() => captureDocument(source, invalid), /端口/);
  assert.equal(source.definition.nodes[0].id, "start");
});
