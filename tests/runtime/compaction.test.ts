...
test("rolling compaction reads every source character and shows only the final summary", async () => {
  ...
  // limit=4000 → maxSummary=1000, reserve≈886 tokens, pass budget ≈ 10899 chars.
  job.snapshot.agent.executionLimits.contextTokens = 4000;
  const transcript = `最初目标\n${"规则和路径/证据😺".repeat(2000)}\n最后一条纠正`;
  ...
  const final = await compactConversation(
    ...
    async (next, emit) => {
      ...
      assert.ok(text.includes("不要逐字照抄"));
      seen.push(text.split(/新增材料（\d+\/\d+，可能从一条消息中间延续）：\n/)[1]);
...
test("material that fits the window compacts in one structured call", async () => {
  const job = agentJob();
  assert.ok(job.snapshot.agent.executionLimits);
  job.snapshot.agent.executionLimits.contextTokens = 32000;
  const transcript = `目标\n${"证据/path 😺".repeat(3000)}`;
  job.compaction = { transcript, focus: "保留路径" };
  let calls = 0;
  let prompt = "";
  const final = await compactConversation(
    job,
    ...
    async (next) => {
      calls++;
      prompt = String(next.messages[0].parts[0].text);
      return {
        id: "checkpoint",
        role: "assistant",
        parts: [{ type: "text", text: "## 用户目标与意图\n- 继续任务" }],
      };
    },
  );
  assert.equal(calls, 1);
  assert.ok(prompt.includes("## 用户目标与意图"));
  assert.ok(prompt.includes("## 关键上下文"));
  assert.ok(prompt.includes("逐字保留文件路径"));
  assert.ok(prompt.includes("本轮压缩关注点（在忠实概括全部材料的前提下优先保留）：保留路径"));
  assert.ok(prompt.includes("对话材料："));
  assert.ok(!prompt.includes("已有摘要"));
  assert.ok(prompt.endsWith(transcript));
  assert.equal(final.id, "checkpoint");
});
...
test("compaction aggregates every pass usage and preserves unknown metrics", async () => {
  ...
  assert.ok(job.snapshot.agent.executionLimits);
  job.snapshot.agent.executionLimits.contextTokens = 4000;
  job.compaction = { transcript: "字".repeat(20000), focus: "" };
