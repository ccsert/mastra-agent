...
测试使用独立 PostgreSQL schema，结束后清理测试 schema。覆盖生成 SDK 到 Mastra 工具执行、隔离/鉴权、发布固定、幂等、取消、失败与租约、消息流竞态。知识库测试另覆盖多批分段、维度传递、重排、删除、隔离和锁等待跨租约截止。浏览器验收见 [M2 记录](docs/development/m2.md)。
...
后续继续接入更多文档格式与可视化处理流程、标准 Skills 包及隔离执行、FlowGram 与 AI 编排、私网 Runtime/连接器和嵌入组件。统一身份中心、细粒度成员权限、内容治理、生产部署与容量验收继续按既定决策推进。当前是开发里程碑，尚未完成企业级生产验收。
