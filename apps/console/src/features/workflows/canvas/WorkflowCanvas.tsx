...
import { WorkflowNodeFields } from "../fields/WorkflowFields";
import {
  type AddableNodeType,
  insertWorkflowNode,
  type NodePlacement,
  pasteWorkflowNodes,
} from "../model/workflow-editing";
import { connectionIssue, needsWorkflowLayout, nodeNames } from "../model/workflow-model";
import {
  scopedWorkflowVariables,
  syncWorkflowVariables,
  type WorkflowVariableOption,
} from "../model/workflow-variables";
import { captureDocument, documentFor, metadataOf, type WorkflowMetadata } from "./document";
import { createNodeRegistries } from "./registry";
import {
  NODE_DRAG_TYPE,
  NodeIcon,
  WorkflowLineInsert,
...
export const WorkflowCanvas = forwardRef<WorkflowCanvasHandle, CanvasProps>(
  function WorkflowCanvas(props, ref) {
    ...
    const nodeRegistries = useMemo(
      () =>
        createNodeRegistries(() => ({
          catalog: live.current.catalog,
          context: context.current,
          outputSchema: source.current.definition.outputSchema,
        })),
      [],
    );
    const capture = (): WorkflowAssetInput =>
      context.current
        ? captureDocument(source.current, context.current.document.toJSON())
        : source.current;
    const notify = () => {
